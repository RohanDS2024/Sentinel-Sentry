...
To prevent resource exhaustion and ensure compliance with system-level file descriptor limits (ulimit), Sentinel-Sentry implements an asyncio.Semaphore. This mechanism caps the number of active, open sockets at any given millisecond, ensuring stability on consumer-grade hardware like the MacBook Air.
...
The following diagram illustrates the flow from target initialization to the final report generation, highlighting how the semaphore manages the task queue.
